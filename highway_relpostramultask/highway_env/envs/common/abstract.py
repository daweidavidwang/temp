...
class AbstractEnv(gym.Env):
    ...
    def _simulateFuture(self,road):
        ...
        ego_vehicles = pd.DataFrame.from_records([road.vehicles[0].to_dict()])[self.features].values.copy()
        ...
        for time in range(10):
            ...
            result_matrix = self.normalize_matrixTrajectory(self.road.vehicles[0],futurePosition)

        return np.array(result_matrix).reshape(100)
    ...
    def normalize_trajectory(self, relativePosition,ego_vehicles,road):
        ...
        relativePosition[0] = utils.lmap(relativePosition[0], [features_range['x'][0], features_range['x'][1]], [-1, 1])
        relativePosition[1] = utils.lmap(relativePosition[0], [features_range['y'][0], features_range['y'][1]], [-1, 1])
    ...
    def normalize_matrixTrajectory(self,ego_vehicles,posMatrix):
        ...
        for i in range(1,int(len(posMatrix[0]) / 2)):
            normalizeMatirx[:,2 * i] = posMatrix[:,2 * i] - posMatrix[:,2 * (i - 1)]
            normalizeMatirx[:,2 * i + 1] = posMatrix[:,2 * i + 1] - posMatrix[:,2 * (i - 1) + 1]

        for i in range(0,int(len(posMatrix[0]) / 2)):
            """normalize x axis"""
            if i == 0:
                normalizeMatirx[:,0] = np.clip(normalizeMatirx[:,0] / 112, -1, 1)
            else:
                normalizeMatirx[:,2 * i] = normalizeMatirx[:,2 * i] /\
                     (0.2 * MDPVehicle.MAX_SPEED)
            """normalize y axis"""
            normalizeMatirx[:,2 * i + 1] = normalizeMatirx[:,2 * i + 1] / (AbstractLane.DEFAULT_WIDTH * len(side_lanes))
