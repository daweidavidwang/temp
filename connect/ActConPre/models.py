...
class MultitaskNetwork(BaseModule, Configurable):
    ...
    def forward(self,currentState,pastPosition,action = None):
        ...
        if action is not None:
            traInputs = torch.cat(
                (pastPosition,action.reshape(action.shape[0],-1)),1
            )
...
if __name__ == "__main__":
    ...
    rloutputs,predictoutputs = model(current_state,pastPosition,action)
    print(rloutputs)
    print(predictoutputs.shape)
