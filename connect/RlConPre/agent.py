...
from abstract import AbstractStochasticAgent
from exploration import exploration_factory
...
class AbstractDQNAgent(AbstractStochasticAgent, ABC):
    ...
    def default_config(cls):
        return dict(model = dict(encoder = dict(in_channels = 5,
                                    in_height = 112,
                                    in_width = 112)),           
    ...
    def record(self, current_state, current_future_pos, current_past_pos,\
        ...
        if batch:
            rl_loss,predict_loss, _, _ = self.compute_bellman_residual(batch)
            self.step_optimizer(rl_loss,self.rl_optimizer)
            self.step_optimizer(predict_loss,self.pre_optimizer)
    ...
    def get_state_action_values(self, current_state, current_past_pos):
        ...
        """
        return self.get_batch_state_action_values([current_state],[current_past_pos])[0]
