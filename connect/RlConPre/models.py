...
class MultitaskNetwork(BaseModule, Configurable):
    def __init__(self, config):
        ...
        self.fc = nn.Linear(self.encoder.convh * self.encoder.convw * 32, 128)

        self.rl = model_factory(self.config["rl_mlp"])
        self.trajectory = model_factory(self.config['predict_mlp'])
    ...
    def rl_updatePara(self):
        rl_list = list(self.encoder.parameters()) + \
            list(self.fc.parameters()) + \
                list(self.rl.parameters())
        return rl_list

    def pre_updatePara(self):
        pre_list = list(self.encoder.parameters()) + \
            list(self.fc.parameters()) + \
                list(self.trajectory.parameters())
        return pre_list
    ...
    def forward(self,currentState,pastPosition):
        ...
        pastPosition = self.fc(pastPosition)
        ...
        traInputs = torch.cat((pastPosition,rlOutputs.clone()),1)
        preOutputs = self.trajectory(traInputs)

        return rlOutputs,preOutputs
...
if __name__ == "__main__":
    ...
    pastPosition = torch.randn(3,5,112,112)
    current_state = torch.randn(3,6,5)
    rloutputs,predictoutputs = model(current_state,pastPosition)
