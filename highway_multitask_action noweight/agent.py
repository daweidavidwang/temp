...
class AbstractDQNAgent(AbstractStochasticAgent, ABC):
    ...
    def default_config(cls):
        return dict(model=dict(type="MultiLayerPerceptron"),
                    ...
                    action_loss_function = "criterion",
    ...
    def record(self, state, action, reward, next_state, done, info):
        ...
        if batch:
            action_loss, value_loss, _, _ = self.compute_bellman_residual(batch)
            self.step_optimizer(action_loss + value_loss)
            self.update_target_network()
            
            self.writer.add_scalar('step/action_loss',action_loss,self.step)
            self.writer.add_scalar('step/value_loss',value_loss,self.step)
