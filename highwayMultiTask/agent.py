...
from abstractagent import AbstractStochasticAgent
...
class AbstractDQNAgent(AbstractStochasticAgent, ABC):
    ...
    def default_config(cls):
        return dict(model=dict(type="vgg19"),
    ...
    def record(self, state, action, reward, next_state, done, trajectory, info):
        """
        ...
        if not self.training:
            return
        self.memory.push(state, action, reward, next_state, done, trajectory, info)
