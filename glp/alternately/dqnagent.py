...
from models import DecisionNetwork, trainable_parameters
...
class DQNAgent(AbstractDQNAgent):
    def __init__(self, env, config=None):
        ...
        self.value_net = DecisionNetwork(self.config["model"])
        self.target_net = DecisionNetwork(self.config["model"])
    ...
    def compute_bellman_residual(self, batch, target_state_action_value=None):
        ...
        current_state_action_values = self.value_net(batch.current_state,batch.current_past_pos)
        ...
        if target_state_action_value is None:
            with torch.no_grad():
                ...
                if self.config["double"]:
                    ...
                    next_state_action_values = self.value_net(batch.next_state,batch.next_past_pos)
                    _, best_actions = next_state_action_values.max(1)
                    # Double Q-learning: estimate action values from target network
                    next_target_state_action_values= self.target_net(batch.next_state,batch.next_past_pos)
                    best_values = next_target_state_action_values.gather(1, best_actions.unsqueeze(1)).squeeze(1)
                else:
                    next_state_action_values = self.target_net(batch.next_state,bacth.next_past_pos)
        ...
        return rl_loss, target_state_action_value, batch
    ...
    def get_batch_state_action_values(self, current_state, current_past_pos):
        values= self.value_net(torch.tensor(current_state, dtype=torch.float).to(self.device),\
