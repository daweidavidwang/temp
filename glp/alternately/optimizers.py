...
def weightLoss(input,target,weight):
    res = 0
    for i in range(input.size(0)):
        first = 0
        second = 0
        first = torch.exp(input[i][target[i]])

        for j in range(input.size(1)):
            second += torch.exp(input[i][j])
        
        res -= torch.log(first / second) * weight[i]
    return res / (target.size(0) * 3)
...
def loss_function_factory(loss_function):
    ...
    elif loss_function == "criterion":
        return F.cross_entropy
    elif loss_function == "weight":
        return weightLoss
    else:
...
def optimizer_factory(optimizer_type, params, lr=None, weight_decay=None, k=None, **kwargs):
    if optimizer_type == "ADAM":
        return torch.optim.Adam(params = filter(lambda p: p.requires_grad,params), lr=lr, weight_decay=weight_decay)
