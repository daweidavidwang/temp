...
class DecisionNetwork(BaseModule, Configurable):
    def __init__(self, config):
        super().__init__()
        Configurable.__init__(self, config)
        self.encoder_path = "./900"
    ...
    def _loadEncoder(self):
        checkpoint = torch.load(os.path.join(self.encoder_path),map_location=torch.device('cpu'))
        pretrained_dict = checkpoint['predict_net']
        encoder_dict = self.encoder.state_dict()
        fc_dict = self.fc.state_dict()
        encoder_state_dict = {k:v for k,v in pretrained_dict.items() if k in encoder_dict.keys()}
        fc_state_dict = {k:v for k,v in pretrained_dict.items() if k in fc_dict.keys()}
        encoder_dict.update(encoder_state_dict)
        fc_dict.update(fc_state_dict)
