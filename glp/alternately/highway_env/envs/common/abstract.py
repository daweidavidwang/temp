...
class AbstractEnv(gym.Env):
    ...
    def reset(self) -> Observation:
        ...
        obs = self.observation_type.observe().reshape(7,5,1)
    ...
    def _simulateFuture(self,road):
        ...
        for _ in range(10):
            ...
            for vehicles in close_vehicles:
                if vehicles in road.vehicles:
                    vehiInfo = pd.DataFrame.from_records([vehicles.to_dict()])[self.features].values.copy()
                    relx = vehiInfo[0][1] - ego_vehicles[0][1]
                    rely = vehiInfo[0][2] - ego_vehicles[0][2]
                    relativePosition = self.normalize_trajectory([relx,rely],road.vehicles[1],road)
                    futurePosition.append(relativePosition)

            for i in range(empty):
                relx = 0.0
                rely = 0.0
                futurePosition.append([0.0,0.0])
