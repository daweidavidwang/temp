...
class DQNAgent(AbstractDQNAgent):
    ...
    def compute_bellman_residual(self, batch, target_state_action_value=None):
        ...
        if not isinstance(batch.state, torch.Tensor):
            state = torch.tensor(np.stack(batch.state),dtype = torch.float).to(self.device)
            ...
            next_state = torch.tensor(np.stack(batch.next_state),dtype = torch.float).to(self.device)
